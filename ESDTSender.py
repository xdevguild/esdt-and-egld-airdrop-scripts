...
airdrop_per_NFT = float(args.amount_airdrop) / ( eligible_holders.shape[0]) - 0.1  

# Compute the weighted airdrop if set to true as an argument
if args.weighted : 
    airdrop_per_holder = float(args.amount_airdrop) / ( eligible_holders.Count.sum() ) - 0.1      
    eligible_holders["Airdrop"] = airdrop_per_holder*data_df.Count
...
for _, row in eligible_holders.iterrows():
    ...
    address = row["Address"] 
    quantity = row["Airdrop"] if args.weighted else airdrop_per_NFT
